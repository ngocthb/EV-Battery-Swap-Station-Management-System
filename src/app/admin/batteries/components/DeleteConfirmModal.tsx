  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await onConfirmAPI(
        Number((deleteModal.data as Battery).id)
      );
      if (response.success) {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Có lỗi xảy ra khi xóa pin";
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
        ...
        <div className="p-6">
          <div className="mb-4">
            <p className="text-gray-600 mb-4">
              Bạn có chắc chắn muốn xóa pin sạc này? Hành động này không thể
              hoàn tác.
            </p>
          ...
          <div className="bg-red-50 border border-red-200 rounded-lg p-3">
            <p className="text-sm text-red-800">
              <strong>Cảnh báo:</strong> Việc xóa pin sẽ xóa tất cả dữ liệu liên
        ...
        <div className="flex justify-end space-x-3 px-6 py-4 border-t border-gray-200 bg-gray-50">
          ...
          >
            ...
            <span>{deleteModal.loading ? "Đang xóa..." : "Xóa pin"}</span>
          </button>
        </div>
      </div>
    </div>
