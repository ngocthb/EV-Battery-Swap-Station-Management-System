...
import { Battery, Cabinet } from "@/types";
...
const RestoreConfirmModal: React.FC<RestoreConfirmModalProps> = ({
  ...
  if (!restoreModal.isOpen || !battery) return null;
  ...
  const handleRestoreConfirm = async () => {
    ...
    try {
      const response = await onConfirmAPI(
        Number((restoreModal.data as Battery).id)
      );
      if (response.success) {
        toast.success(response.message || "Khôi phục pin thành công!");
        refreshList();
        dispatch(closeRestoreModal());
      } else {
        toast.error(response.message || "Khôi phục pin thất bại!");
      }
    } catch (error: unknown) {
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Có lỗi xảy ra khi khôi phục pin";
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
        ...
        <div className="p-6">
          <div className="mb-4">
            <p className="text-gray-600 mb-4">
              Bạn có chắc chắn muốn khôi phục pin sạc này? Pin sẽ được kích hoạt
              trở lại và có thể phục vụ khách hàng.
            </p>
            ...
            <div className="bg-gray-50 rounded-lg p-4 space-y-3">
              <div>
                ...
                <p className="text-gray-900">{battery?.model}</p>
              </div>
          ...
          <div className="bg-green-50 border border-green-200 rounded-lg p-3">
            <p className="text-sm text-green-800">
              <strong>Lưu ý:</strong> Sau khi khôi phục, pin sẽ chuyển sang
        ...
        <div className="flex justify-end space-x-3 px-6 py-4 border-t border-gray-200 bg-gray-50">
          ...
          >
            ...
            <span>
              {restoreModal.loading ? "Đang khôi phục..." : "Khôi phục pin"}
            </span>
          </button>
        </div>
      </div>
