...
function StatsList({ slotList }: { slotList: Slot[] }) {
  const stats = [
    {
      label: "Tổng ô",
      icon: <Box className="w-6 h-6 text-blue-600" />,
      bgColor: "bg-blue-100",
      value: slotList?.length || 0,
    },
    {
      label: "Hoạt động",
      icon: <Box className="w-6 h-6 text-green-600" />,
      bgColor: "bg-green-100",
      value: slotList?.filter((c) => c?.status === "AVAILABLE").length || 0,
    },
    {
      label: "Ngưng hoạt động",
      icon: <Box className="w-6 h-6 text-red-600" />,
      bgColor: "bg-red-100",
      value: slotList?.filter((c) => c?.status === "MAINTENANCE").length || 0,
    },
  ];
