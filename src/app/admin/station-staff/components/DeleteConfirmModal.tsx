...
const DeleteConfirmModal: React.FC<DeleteConfirmModalProps> = ({
  ...
  if (!deleteModal.isOpen || !user) return null;
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await onConfirmAPI((deleteModal.data as User).id);
      if (response.success) {
        ...
        <div className="p-6">
          ...
          <div className="bg-red-50 border border-red-200 rounded-lg p-3">
            <p className="text-sm text-red-800">
              <strong>Cảnh báo:</strong> Việc xóa nhân viên sẽ xóa tất cả dữ
              liệu liên quan và không thể khôi phục.
            </p>
          </div>
        </div>
